 ...
 */
public class ExportJdrCommand implements Command<ExportJdrRequest, GenericSuccessResponse> {
    public static final Class<ExportJdrRequest> REQUEST_CLASS = ExportJdrRequest.class;

    @Override
    public BasicMessageWithExtraData<GenericSuccessResponse> execute(ExportJdrRequest request, BinaryData binaryData,
        ...
        try (ConnectionContextFactory ccf = new ConnectionContextFactory(context.getConnectionFactory())) {
            Endpoint endpoint = Constants.DEST_FEED_EXPORT_JDR;
            ...
            response.setMessage("The Export JDR request has been forwarded to feed [" + feedId + "] (id=" + mid + ")");
